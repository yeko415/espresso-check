(() => {
  ...
  // Scroll-driven horizontal scene: vertical scroll becomes horizontal motion while the stage is pinned.
  const pinScene = qs('.pin-marquee');
  const pinRail = qs('#pinRail');
  const floaters = qsa('[data-float]');
  let sceneScroll = 0, sceneTarget = 0;
  function animatePinScene() {
    if (pinScene && pinRail && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      const rect = pinScene.getBoundingClientRect();
      const range = Math.max(1, pinScene.offsetHeight - window.innerHeight);
      const progress = Math.min(1, Math.max(0, -rect.top / range));
      sceneTarget = progress;
      sceneScroll += (sceneTarget - sceneScroll) * 0.1;
      const maxX = Math.max(0, pinRail.scrollWidth - window.innerWidth);
      pinRail.style.transform = `translate3d(${-sceneScroll * maxX}px,0,0)`;
      floaters.forEach((el, i) => {
        const d = Number(el.dataset.float || 0.1);
        const drift = (sceneScroll - .5) * d * 180;
        el.style.transform = `translate3d(${drift}px, ${Math.sin(sceneScroll * Math.PI * 2 + i) * 10}px, 0) rotate(${(i ? 1 : -1) * sceneScroll * 7}deg)`;
      });
    }
    requestAnimationFrame(animatePinScene);
  }
  requestAnimationFrame(animatePinScene);

  // Desktop wheel gesture over a horizontal rail: trackpad/mouse wheel nudges the carousel without changing page architecture.
  const productWindow = qs('.carousel-window');
  productWindow?.addEventListener('wheel', e => {
    if (Math.abs(e.deltaY) <= Math.abs(e.deltaX)) return;
    const max = productWindow.scrollWidth - productWindow.clientWidth;
    if (max <= 0) return;
    e.preventDefault();
    window.scrollBy({ top: e.deltaY * .35, behavior: 'auto' });
  }, { passive: false });
